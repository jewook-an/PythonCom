...
class DatabaseManager:
    ...
    def get_by_id(self, model: Any, id: Any) -> Optional[Base]:
        """
        ID로 단일 아이템 조회
        """
        try:
    ...
    def get_all(self, model: Any, skip: int = 0, limit: int = 100) -> List[Base]:
        """
        모든 아이템 조회 (페이지네이션 지원)
        """
        try:
    ...
    def update_item(self, model: Any, id: Any, update_data: Dict[str, Any]) -> Optional[Base]:
        """
        아이템 업데이트
        """
        try:
    ...
    def get_by_filter(self, model: Any, filters: Dict[str, Any]) -> List[Base]:
        """
        필터 조건으로 아이템 조회
        """
        try:
